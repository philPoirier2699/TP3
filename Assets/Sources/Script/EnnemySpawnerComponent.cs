...
public class EnnemySpawnerComponent : MonoBehaviour
{
    ...
    private float fasterSpawn = 0;

    public double maxSpawnReduce;


    private void Awake()
    {
        maxSpawnReduce = timeBetweenSpawns - 0.25;
    }
    void Update()
    {
        if (fasterSpawn < maxSpawnReduce)
            fasterSpawn += Time.deltaTime * Time.deltaTime;
        elapsed += Time.deltaTime;
        if(elapsed > timeBetweenSpawns - fasterSpawn)
        {
            Spawn();
            elapsed = 0;
        }
